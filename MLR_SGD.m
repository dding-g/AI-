%Mulinomiar Logistic Regression
function [th, train_accuracy, train_loss] = MLR_SGD(theta, Xtrain, Ytrain,alpha)
...
for i=1:m
  z = Xtrain(i,:)*th;
  py_x = softmax(z);
  th(:,1)=th(:,1) - (alpha)*(py_x(1)-(Ytrain(i)==1))*Xtrain(i,:)';
  th(:,2)=th(:,2) - (alpha)*(py_x(2)-(Ytrain(i)==2))*Xtrain(i,:)';
  th(:,3)=th(:,3) - (alpha)*(py_x(3)-(Ytrain(i)==3))*Xtrain(i,:)';
end
